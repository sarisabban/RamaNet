...
# 1. Download PDB Database
# 2. Extract Files
# 3. Separate all chains
# 4. Remove structures less than 80 AA and larger than 150 AA
# 5. Remove structure with broken chains
# 6. Remove structures that have loops that are larger than a spesific number (5 AA)
# 7. Measure RMSD of each structure to each structure, remove if RMSD < 0.5
# 8. measure distances between 1 AA and all the others then put results in a table
# 9. manually classify tables for open conformation structures ---> maybe make a dataset of 1000 examples? 500 good 500 bad and use KNN to continue the remaining classification? (this uses the CA XYZ coordinates)
