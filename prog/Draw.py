...
def ConstructPDB(filename):
	...
	for line in data:
		if tag == '+':
			...
			Oz = line[8]
			OCx = round(float(line[6]) + 1.773 , 3)
			OCy = round(float(line[7]) - 1.691 , 3)
			OCz = line[8]
			...
			AtoCount += 1
			TheOCLine = '{:6}{:5d} {:4}{:1}{:3} {:1}{:4d}{:1}   {:8}{:8}{:8}{:6}{:6}          {:2}{:2}'.format('ATOM' , AtoCount , 'OC' , '' , 'GLY' , 'A' , ResCount , '' , OCx , OCy , OCz , 1.0 , 0.0 , 'O' , '') + '\n'
			AtoCount += 1
			ResCount += 1
...
def RotatePDB(line):
	...
	RCA = numpy.array([1.408 , 31.978])
	LCA = numpy.array([0.217 , 30.077])
	...
	scaled_u = (2.4 / 3.8) * u
	redtop = RCA + scaled_u
	print('C = ' , (round(redtop[0] , 3) , round(redtop[1] , 3)))
	...
	scaled_u = (2.4 / 3.8) * u
	redtop = LCA + scaled_u
	print('N = ' , (round(redtop[0] , 3) , round(redtop[1] , 3)))
