...
class Design():
	...
	def Pack(Pose):
		''' Applies FastRelax to change the whole structure's amino acids (one layer at a time as well as designing towards an optimally packed core) while maintaining the same backbone. Should be faster than the Whole method and results in a better final structure than the Layer method '''
		...
		for identity in layers:										#Loop through each layer
			...
			mover = pyrosetta.rosetta.protocols.relax.FastRelax()					#Call the FastRelax Mover
