...
def FoldPDB_PS(line):
	''' Fold a primary structure using the phi and psi torsion angles '''
	''' Generates the Backbone.pdb file '''
	size = int(len(line.split(';')) / 2)
	Vs = list()
	for numb in range(size):
		Vs.append('V')
	sequence = ''.join(Vs)
	pose = pose_from_sequence(sequence)
	#Isolate each angle type
	tick = 0
	PHI = list()
	PSI = list()
...
def FoldPDB_PSO(line):
	''' Fold a primary structure using the phi, psi, and omega torsion angles '''
	''' Generates the Backbone.pdb file '''
